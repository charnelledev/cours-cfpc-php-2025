<?php
session_start();
require_once "database.php"; // Assurez-vous que ce fichier contient la connexion à votre base de données

// Vérification du rôle de l'utilisateur

if ($_SESSION['role'] != 'admin') {
  header("Location: index.php");  
}

// Traitement de l'ajout d'un étudiant
if (isset($_POST['create']) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
  ...
  if (!empty($pseudo) && filter_var($mail, FILTER_VALIDATE_EMAIL)) {
      ...
      $_SESSION['message'] = "Étudiant ajouté avec succès !";
...
// Récupérer la liste des étudiants
...
<head>
  ...
  <title>Gestion des étudiants</title>
  <!-- Intégration de Tailwind CSS v4 -->
  <script src="https://cdn.tailwindcss.com"></script>
</head>

<body class="bg-green-50 p-14">
  <div class="container mx-auto p-4">
    <h1 class="text-3xl font-bold text-green-700 text-center mb-6">Gestion des étudiants</h1>
    ...
    <!-- Formulaire d'ajout d'un étudiant -->
    <div class="bg-white shadow-lg rounded-lg p-10 mb-6">
      <h2 class="text-2xl font-bold text-green-700 mb-4">Ajouter un étudiant</h2>
      <form action="gestion_students.php" method="POST" class="space-y-4">
        <input type="hidden" name="action" value="add">
        <div>
          <label for="name" class="block text-green-700 font-medium">Nom :</label>
          <input type="text" id="name" name="pseudo" placeholder="Nom de l'étudiant"
        ...
        <div>
          ...
          <input type="email" id="email" name="mail" placeholder="Email de l'étudiant"
            class="w-full px-4 py-2 border border-green-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
    ...
    <div class="bg-white shadow-lg rounded-lg p-10">
      <h2 class="text-2xl font-bold text-green-700 mb-4">Liste des étudiants</h2>
        ...
        <tbody class="bg-white divide-y divide-green-100">
          ...
          <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
              <!-- Liens pour éditer et supprimer (pages à créer séparément) -->
